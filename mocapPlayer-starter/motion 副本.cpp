...
int Motion::readAMCfile(char* name, double scale)
{
  ...
  if( file.fail() ) 
    ...
    // Analysis
    // **********
    std::ofstream an("/Users/Winters/Downloads/csci520/mocapPlayer-starter/intput600800.txt");
    if(an.fail()) 
        return -1;
    // **********
      ...
      for(int x = 0; x < bone[bone_idx].dof; x++)
      {
        ...
        case 3:
          m_pPostures[i].bone_rotation[bone_idx].p[2] = tmp;
                // Analysis
                // **********
                if(bone_idx == 2 && i >= 600 && i<= 800)
                {
                    an << i << " " << m_pPostures[i].bone_rotation[bone_idx].p[0] << std::endl;
                }
                // **********
          break;
        case 4:
...
int Motion::writeAMCfile(char * filename, double scale, int forceAllJointsBe3DOF)
{
  ...
  if(os.fail()) 
    ...
    // Analysis
    // **********
    std::ofstream an("/Users/Winters/Downloads/csci520/mocapPlayer-starter/1outputdata_bq20.txt");
    if(an.fail()) 
        return -1;
    // **********
        ...
        for(int d=0; d<bone[j].dof; d++)
        {
          ...
          if (bone[j].dofo[d] == 1)
          {
            ...
            if(bone[j].dofrx == 1) 
              ...
              if(j == 2 && f >= 600 && f<= 800)
              {
                  an << f << " " << m_pPostures[f].bone_rotation[j].p[0] << std::endl;
              }
              // **********
          }
